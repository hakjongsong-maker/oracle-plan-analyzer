...
def load_config() -> dict:
    if not CONFIG_PATH.exists():
        ...
        print("  mail_config.json 을 생성하고 SMTP 정보를 입력하세요.")
        sys.exit(1)
...
def build_message(cfg: dict, part_path: Path, part_num: int, total_parts: int) -> MIMEMultipart:
    """단일 파트 파일에 대한 이메일 메시지 생성."""
    msg = MIMEMultipart()
    ...
    body = f"""\
OraclePlanAnalyzer 분할 배포 파일입니다.

  파일명  : {part_path.name}
  크기    : {size_mb:.1f} MB
  파트    : {part_num} / {total_parts}

-----------------------------------------
[ 전체 파일 수신 후 사용 방법 ]
1. 모든 파트 파일 ({total_parts}개) + combine.bat 을 같은 폴더에 저장
2. combine.bat 실행 -> OraclePlanAnalyzer.zip 생성
3. zip 압축 해제 -> OraclePlanAnalyzer.exe 실행
...
def send_parts(part_files: list[Path], delay_sec: int = 3) -> bool:
    ...
    delay_sec: 메일 간 발송 간격 (Gmail 전송 제한 방지)
    """
    cfg = load_config()
